        ...
        $rootNode
            ->children()
                ...
                ->arrayNode('apis')
                    ...
                    ->children()
                        ->scalarNode('schema')
                            ->info('Absolute path to the OpenAPI/Swagger2.0 schema')
                            ->isRequired()
                        ->end()
                        ->scalarNode('client')
                            ->info('Use a specific HTTP client for an API Service')
                            ->defaultValue('api_service.client')
                        ->end()
                        ->scalarNode('logger')
                        ...
                        ->arrayNode('cache')
                            ...
                            ->children()
                                ->scalarNode('service')
                                    ->info('The service Id that should be used for caching schemas')
                                    ->isRequired()
                                ->end()
                            ->end()
                        ->end()
                        ->arrayNode('config')
                            ...
                            ->children()
                                ->scalarNode('baseUri')
                                    ->isRequired()
                                    ->info('The uri of your service (ex: http://domain.tld)')
                                ->end()
                                ->scalarNode('validateRequest')
                                    ->defaultTrue()
                                    ->info('Validate the request before sending it')
                                ->end()
                                ->scalarNode('validateResponse')
                                    ->defaultTrue()
                                    ->info('Validate the response before sending it')
                                ->end()
                                ->scalarNode('returnResponse')
                                    ->defaultFalse()
                                    ->info('Return a Response object instead of a resource')
                                ->end()
                            ->end()
                        ->end()
                    ->end()
